...
[c,l] = wavedec(X,2,'db4');
a2X = wrcoef('a',c,l,'db4',2);
d1X = wrcoef('d',c,l,'db4',1);
d2X = wrcoef('d',c,l,'db4',2);
[c,l] = wavedec(T,2,'db4');
a2T = wrcoef('a',c,l,'db4',2);
d1T = wrcoef('d',c,l,'db4',1);
d2T = wrcoef('d',c,l,'db4',2);

X = num2cell(d2X(1:138));
T = num2cell(d2T(1:138));
Xnew = num2cell(d2X(139:162));
Tnew = d2T(139:162);
...
for delay = 1:A(1)
    ...
    for numHiddenNeurons = 1:A(2)
        ...
        for run = 1:A(3)
            ...
            net = narxnet(1:1,1:delay,numHiddenNeurons); %narxnet(inputDelays,feedbackDelays,hiddenSizes)
            ...
            predictValMatrix(j,1:length(tr.valRatio)) = predictRatio;
            ...
            [Xs1,Xi1,Ai1,Ts1] = preparets(net,X,{},T);
            [Y1,Xf,Af] = net(Xs1,Xi1,Ai1);
            [netc,Xic,Aic] = closeloop(net,Xf,Af);
            
            view(netc)
            
            Yc = netc(Xnew,Xic,Aic);
            
            targetTest = cell2mat(Tnew);
            targetTestMatrix(j,1:length(Tnew)) = targetTest;
            predictTest = cell2mat(Yc);
            predictTestMatrix(j,1:length(Tnew)) = predictTest;
            ...
            performance(delay*A(3)-A(3)+run,numHiddenNeurons*18-18+9) = (mean(abs(errorVal)/abs(targetVal)))*100;
